...
public class MainClass implements BookOperation {
    ...
    private Book[] bookList = new Book[MAXNUM];
    ...
    public void deleteBook() {
        ...
        for(int i = 0; i<bookNum; i++){
            if (bookName.equals(bookList[i].getName())){
    ...
    public void changeBook() {
        ...
        for(int i = 0; i<bookNum; i++){
            if (bookName.equals(bookList[i].getName())){
                ...
                bookList[i].setName(newBookName);

                System.out.println("请输入新作者");
                String newBookAuthor=sc.nextLine();
                bookList[i].setAuthor(newBookAuthor);

                System.out.println("请输入新价格");
                double newBookPrice=Double.parseDouble(sc.nextLine());
                bookList[i].setPrice(newBookPrice);
    ...
    public void searchBook() {
        ...
        for (int i = 0; i < bookNum; i++) {
            if (bookList[i].getName().contains(keyword)) {
                System.out.println("  " + (i + 1) + ". " + bookList[i].print());
                found = true;
            }
        }
        if (!found) {
